...
export async function GET(request: NextRequest) {
  ...
  if (!email || !token) {
    return htmlResponse("Invalid Link", "This unsubscribe link is invalid.", 400);
  }
  ...
  try {
    ...
    return htmlResponse(
      "Unsubscribed",
      "You've been unsubscribed and won't receive any more emails from OpenClaw DFW. Sorry to see you go!",
      200
    );
  } catch (error) {
